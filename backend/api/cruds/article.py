    ...
    try:
        async with db.begin():
            for article_create in articles_create:
                ...
                article_data["updated_at"] = current_time

                stmt = mysql_insert(article_model.Article).values(**article_data)
                do_update_stmt = stmt.on_duplicate_key_update(updated_at=current_time)
...
async def get_articles(db: AsyncSession) -> List[article_schema.ArticleResponse]:
    ...
    try:
        ...
        for article in articles:
            # モデル変換
            tags_converted = [tag_schema.TagRespose.model_validate(tag) for tag in article.tags]
